local function _onequip(inst, owner, build, symbol_override,
                        headbase_hat_override, opentop, helm)
  if type(symbol_override) ~= "string" then symbol_override = "swap_hat" end
  local skin_build = inst:GetSkinBuild()
  build = build or inst.AnimState:GetBuild()
  if skin_build ~= nil then
    owner:PushEvent("equipskinneditem", inst:GetSkinName())
    owner.AnimState:OverrideItemSkinSymbol("swap_hat", skin_build,
                                           symbol_override, inst.GUID)
  ...
  if headbase_hat_override ~= nil and build ~= nil then
