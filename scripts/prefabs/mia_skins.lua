...
local function fmt(tb, name, override)
  local t = shallowcopy(tb)
  for i, v in pairs(t) do
    if override and override[i] then
      t[i] = override[i]
    else
      t[i] = type(v) == "string" and v:format(name) or (type(v) == "table" and fmt(v, name) or v)
    end
  end
  return t
end
for name, v in pairs(chars) do
  local this_param = fmt(params, name, v)
