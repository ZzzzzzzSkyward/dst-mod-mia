...
local function OnIndicatorUnequipped(inst)
    inst.widget:Hide()
end

local ChargeBadge = Class(Badge, function(self, owner, art)
    Badge._ctor(self, art, owner, {174 / 255, 21 / 255, 21 / 255, 1}, "status_charge", nil, nil, true)
    ...
    self.effigyanim.inst:ListenForEvent("animover", OnIndicatorUnequipped)
    self.effigy = false
...
function ChargeBadge:SetPercent(val, max, penaltypercent)
...
function ChargeBadge:OnUpdate(dt)
    if TheNet:IsServerPaused() then return end

    local down
    if (self.owner.IsFreezing ~= nil and self.owner:IsFreezing())
        or (self.owner.replica.health ~= nil and self.owner.replica.health:IsTakingFireDamageFull())
        or (self.owner.replica.hunger ~= nil and self.owner.replica.hunger:IsStarving()) or next(self.corrosives) ~= nil then
        down = "_most"
    ...
    local up = down == nil
                   and (((self.owner.player_classified ~= nil and self.owner.player_classified.issleephealing:value())
                       or next(self.hots) ~= nil
                       or (self.owner.replica.inventory ~= nil and self.owner.replica.inventory:EquipHasTag("regen")))
                       or (self.owner:HasDebuff("wintersfeastbuff"))) and self.owner.replica.health ~= nil
                   and self.owner.replica.health:IsHurt()

    local anim = (down ~= nil and ("arrow_loop_decrease" .. down)) or (not up and "neutral")
                     or (next(self.hots) ~= nil and "arrow_loop_increase_most") or "arrow_loop_increase"
