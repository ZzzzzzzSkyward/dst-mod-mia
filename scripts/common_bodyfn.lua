local function _onequip(inst, owner, build, symbol_override)
  if type(symbol_override) ~= "string" then symbol_override = inst.bodysymbol or "swap_body" end
  if type(build) ~= "string" then build = inst.build or "armor_wood" end
  local skin_build = inst:GetSkinBuild()
  if skin_build ~= nil then
    owner:PushEvent("equipskinneditem", inst:GetSkinName())
    owner.AnimState:OverrideItemSkinSymbol("swap_body", skin_build, symbol_override, inst.GUID, build)
  else
  ...
  if inst._onblocked then inst:ListenForEvent("blocked", inst._onblocked, owner) end
...
local function _onunequip(inst, owner)
  ...
  if inst._onblocked then inst:RemoveEventCallback("blocked", inst._onblocked, owner) end
